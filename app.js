// ROTOMI Website — Multi-product Brand Site
...
class ROTOMIWebsite {
    ...
    handleFormSubmit(e) {
        ...
        fetch(form.action || 'send-mail.php', {
            method: 'POST',
            body: formData
        })
        .then(res => res.json())
        .then(data => {
            ...
            if (data.success) {
                form.reset();
                this.showFormMessage(form, data.message || 'Thank you! We\'ll be in touch within 24 hours.', 'success');
            } else {
                const msg = data.errors ? data.errors.join(' ') : (data.error || 'Something went wrong. Please try again.');
                this.showFormMessage(form, msg, 'error');
            }
        })
        .catch(() => {
            btn.textContent = originalText;
